...
char *convertToBase32(unsigned int num)
{
	...
	base32_seq[0] = base32[extractBits(num, 5, 9)];
	base32_seq[1] = base32[extractBits(num, 0, 4)];
	base32_seq[2] = '\0';
 ...
 */
char *nextListToken(char *dest, char *line)
{
	char *temp = dest;

	if(endOfLine(line)) /* If the given line is empty, copy an empty token */
	{
		dest[0] = '\0';
		return NULL;
	}
	...
	while(!endOfLine(line) && *line != ',' && !isspace(*line))
	{
		*temp = *line;
		temp++;
		line++;
 ...
 */
char *nextTokenString(char *dest, char *line)
{
	char temp[LINE_LENGTH];
	line = nextListToken(dest, line);
	if(*dest != '"') return line;
	while(!endOfLine(line) && dest[strlen(dest) - 1] != '"')
	{
		line = nextListToken(temp, line);
		if(line)
			strcat(dest, temp);
	}
	return line;
...
char *nextToken(char *seq)
{
	if(seq == NULL)
		return NULL;
	while(!isspace(*seq) && !endOfLine(seq)) 
		seq++; /* Skip rest of characters in the current token (until a space) */
	seq = skipSpaces(seq); /* Skip spaces */
	if(endOfLine(seq)) 
		return NULL;
	return seq;
}
...
void copyToken(char *dest, char *line)
{
	int i = 0;
...
int findIndex(char *token, const char *arr[], int n)
{
	int i;
	for(i = 0; i < n; i++)
...
boolean isRegister(char *token)
{
    ...
    return strlen(token) == REGISTER_LENGTH && token[0] == 'r' && token[1] >= '0' && token[1] <= '7';
}

/* Check if a token matches a directive name */
int findDirective(char *token)
{
	if(token == NULL || *token != '.') 
		return NOT_FOUND;
	return findIndex(token, directives, NUM_DIRECTIVES);
}
...
int findCommand(char *token)
{
	...
	if(token_len > MAX_COMMAND_LENGTH || token_len < MIN_COMMAND_LENGTH)
		return NOT_FOUND;
	return findIndex(token, commands, NUM_COMMANDS);
}
